...
The [dashboard](dashboard:moj9XTy) shows a view of the input PDFs and output files. Documents can be uploaded by clicking on the small arrow to the right of "Content of PDF_Input" and then "Go to source managed folder" in the top right, followed by dragging and dropping files on to the left of the interface. You can then return to the dashboard using "G+P" on the keyboard. The [scenario](scenario:ProcessDocument) to process the input PDFs can also be triggered from here. Please see below for further information.
